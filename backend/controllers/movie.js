...
exports.uploadTrailer = async (req, res)=>{
    ...
    if(!videoFile) return res.status(401).json({"error": "video file is missing"});

    const {public_id, secure_url} = await cloudinary.v2.uploader.upload(videoFile.path, {resource_type: 'video'});
...
exports.createMovie = async (req, res)=>{
  ...
  if(writers){
    ...
    for(let writerId of writers){
      if(isValidObjectId(writerId)) return res.status(401).json({"error": "invalid writer id at index ", index});
      index++;
    }
  ...
  const {secure_url, public_id, responsive_breakpoints} = await cloudinary.v2.uploader.upload(file.path, {
    transformation: {
      width: 1280,
      height: 720
    },
...
exports.removeMovie = async (req, res)=>{
  ...
  if(!movieToDelete) return res.status(404).json({"error": "movie not found to delete"});

  const posterId = movieToDelete.poster?.public_id;

  if(posterId){
    const {result} = await cloudinary.v2.uploader.destroy(posterId);
    if(result !== 'ok') return res.status(401).json({"error": "could not remove poster from cloudinary, hence movie will not be deleted from mongodb"});
  }

  const trailerId = movieToDelete.trailer?.public_id;

  if(!trailerId) return res.status(401).json({"error": "could not find trailer, hence not deleting movie completely"});

  if(trailerId){
    const {result} = await cloudinary.v2.uploader.destroy(trailerId, {resource_type: 'video'});
    if(result !== 'ok') return res.status(401).json({"error": "could not remove trailer from cloudinary, hence movie will not be deleted from mongodb"});
  }
