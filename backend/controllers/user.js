...
exports.verifyEmail = async (req, res) => {
    ...
    if(!user){
        return res.status(404).json({"error": "user does not exist but the id does, hence can't be verified"});
    }
    ...
    console.log("I have reached here to delete the token from email verification token")
...
exports.resendEmailVerificationToken = async (req, res) => {
    ...
    if(user.isVerified){
        return res.json({"error": "This email is already verified, no need to try to re verify this email"});
    }
    ...
    if(alreadyHasToken){
        return res.json({"error": "One OTP has already been sent to you. Wait one another to request for another OTP"});
    }
    ...
    res.status(201).json({"message": "Please verify your email. Check your inbox for the new OTP."});
};
...
exports.forgotPassword = async (req, res) => {
    ...
    const alreadyHasPasswordResetToken = await PasswordResetToken.findOne({owner: user._id});
    if(alreadyHasPasswordResetToken){
        return res.status(401).json({"error": "Wait an hour to regenerate another password reset token"});
    }
    ...
    const newPasswordResetToken = await new PasswordResetToken({owner: user._id, token: tokenToResetPassword});
    ...
    res.json({"message": "Link to reset your password has been sent to your email"});
};
...
exports.resetPassword = async(req, res)=>{
    ...
    console.log("User for which we are resetting the password = " + userWhosePasswordHasToBeUpdated);
    ...
    const passwordIsSame = await userWhosePasswordHasToBeUpdated.comparePassword(newPassword);
    console.log(passwordIsSame);
    if(passwordIsSame){
        return res.status(401).json({"error": "new password should be different from the old one"});
    }
