...
exports.createActor = async (req, res)=>{
    ...
    if(imageFile){
        const {secure_url, public_id} = await cloudinary.v2.uploader.upload(imageFile.path, {gravity: "face", height: 500, width: 500, crop: "thumb"});
...
exports.updateActor = async (req, res)=>{
    ...
    if(!isValidObjectId(actorId)) return res.status(401).json({"error": "The id present in the url params is not a valid object id or it might be undefined/null"});

    const actorToBeUpdated = await Actor.findById(actorId);

    if(!actorToBeUpdated) return res.status(401).json({"error": "There is no actor is the database corresponding to the id in the url parameters"});

    const public_id = actorToBeUpdated.avatar?.public_id;

    if(public_id && updatedImageFile){
        const {result} = await cloudinary.v2.uploader.destroy(public_id);
        if(result !== 'ok')  return res.status(401).json({"error": "Could not delete the image from cloudinary with id " + public_id +" for updation purpose."});
    }

    if(updatedImageFile){
        const {secure_url, public_id} = await cloudinary.v2.uploader.upload(updatedImageFile.path, {gravity: "face", height: 500, width: 500, crop: "thumb"});
...
exports.removeActor = async (req, res)=>{
    ...
    if(!actorToBeDeleted) return res.status(401).json({"error": "There is no actor is the database to delete corresponding to the id in the url parameters"});

    const public_id = actorToBeDeleted.avatar?.public_id;

    if(public_id){
        const {result} = await cloudinary.v2.uploader.destroy(public_id);
        if(result !== 'ok')  return res.status(401).json({"error": "Could not delete the image from cloudinary with id", public_id});
    }
...
exports.getLatestActors = async (req, res)=>{
    ...
    const formattedLatestActorsArray = latestActors.map((actor)=>formatActor(actor));
...
exports.getSingleActor = async (req, res)=>{
    ...
    if(!isValidObjectId(actorId)) return res.status(401).json({"error": "The id present in the url params to search for a single actor is not a valid object id or it might be undefined/null"});

    const actor = await Actor.findById(actorId);

    if(!actor) return res.status(404).json({"error": "No actor was found in the db corresponding the the id present in the url params to get a single actor"});
