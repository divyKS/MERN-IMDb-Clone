...
exports.userValidator = [
  ...
  check("password")
    ...
    .withMessage("Password length needs to be in [4,10]"),
];
...
exports.validateMovie = [
  ...
  check("storyLine")
    ...
    .not()
    .isEmpty()
    .withMessage("Movie story line is missing"),
  check("language")
    .trim()
    .not()
    .isEmpty()
    .withMessage("Movie language is missing"),
  ...
  check("status")
    .isIn(["public", "private"])
    .withMessage("Movie status can be public or private"),
  check("genres")
    .isArray()
    .withMessage("Generes must be an array of strings!")
    .custom((genreArrayFromFrontend) => {
  ...
  check("tags")
    ...
    .custom((tagsArrayFromFrontend) => {
      for (let tag of tagsArrayFromFrontend) {
        if (typeof tag !== "string")
          throw Error("Each individual tag has to be a string");
      }
      return true;
    }),
  check("cast")
    ...
    .custom((castFromFrontend) => {
      for (let item of castFromFrontend) {
        ...
        if (typeof item.leadActor !== "boolean")
          throw Error(
            "Only boolean values are accepted inside the lead actor attribute of cast",
          );
      }
      return true;
    })
