...
const EmailVerification = () => {
  ...
  const handleOTPChange = (e, index)=>{
    ...
    updatedOtp[currentOTPIndex] = value.substring(value.length - 1, value.length); // because of this only the last things that we entered comes into the otp, we can now not more than 1 digit in one box
    if(!value){
  ...
  const handleSubmit = async (e)=>{
    ...
    if(!isValidOTP(otp)){
      return useNotifcation.updateNotification('error', 'Enter all 6 digits for the OTP')
    }
    console.log(user)
    const {error, message, user: userResponse} = await verifyUserEMail({"OTP": otp.join(''), "userId": user.id}) // capital OTP since that is how we destructured it in our backend function
    if(error){
